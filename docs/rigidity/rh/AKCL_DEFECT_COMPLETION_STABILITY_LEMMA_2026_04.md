# AKCL Defect Completion Stability Lemma (2026-04)
...
Prove that the theorem-level defect functional is stable under this completion:
\[
f_n \to f \text{ in } \mathcal A_{\mathrm{AKCL}}
\quad\Longrightarrow\quad
\operatorname{Def}_{\mathrm{AKCL}}[f_n] \to \operatorname{Def}_{\mathrm{AKCL}}[f].
\]

## Role in the transfer chain
This is Obligation 3 from
`docs/rigidity/rh/AKCL_TRANSFER_OBLIGATIONS_REGISTRY_2026_04.md`.

## Exact missing objects
1. The exact theorem-level defect functional \(\operatorname{Def}_{\mathrm{AKCL}}[\cdot]\).
2. The exact theorem-level norm \(\|\cdot\|_{\mathcal A_{\mathrm{AKCL}}}\).
3. A continuity or closed-form extension argument for \(\operatorname{Def}_{\mathrm{AKCL}}\) on the completion.
...
1. an exact formula-level definition of \(\operatorname{Def}_{\mathrm{AKCL}}[\cdot]\);
2. an exact formula-level definition of \(\|\cdot\|_{\mathcal A_{\mathrm{AKCL}}}\);
3. a proof or certified verifier establishing completion stability of the defect functional.
